...
def game_init():
    ...
    for i in range(0, 10):
        for j in range(0, 10):
            b = Button(root, height=2, width=4, relief='groove',
                       command=lambda id=bid: judgment(id))
            b.grid(row=i+4, column=j)
            btns.append(b)
            bid += 1
    fontsize = tkinter.font.Font(family="Arial", size=13)
    Label(root, textvariable=LevelText, font=fontsize).grid(
        row=0, columnspan=10)
    Label(root, textvariable=timeText, font=fontsize).grid(row=1, columnspan=10)
    Label(root, textvariable=ScoreText, font=fontsize).grid(
        row=2, columnspan=10)
    bnttext_set()
    run_game()
