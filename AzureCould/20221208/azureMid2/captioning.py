...
class Captioning(object):
    ...
    def adjust_real_time_caption_text(self, text: str, is_recognized_result: bool) -> str:
        ...
        temp_caption_helper = caption_helper.CaptionHelper(
            self._user_config["language"], self._user_config["max_line_length"], self._user_config["lines"], [])
    ...
    def caption_from_real_time_result(self, result: speechsdk.SpeechRecognitionResult, is_recognized_result: bool) -> Optional[str]:
        ...
        if self._previous_end_time is not None and self._previous_end_time > end_time:
            ...
            caption = caption_helper.Caption(self._user_config["language"], self._srt_sequence_number, helper.add_time_and_timedelta(
                start_time, self._user_config["delay"]), helper.add_time_and_timedelta(end_time, self._user_config["delay"]), "")
            ...
            if self._previous_caption is not None:
                ...
                if self._previous_result_is_recognized:
                    ...
                    previous_end = helper.add_time_and_timedelta(
                        self._previous_caption.end, self._user_config["remain_time"])
    ...
    def captions_from_offline_results(self) -> List[caption_helper.Caption]:
        captions = caption_helper.get_captions(
            self._user_config["language"], self._user_config["max_line_length"], self._user_config["lines"], list(self._offline_results))
        # Save the last caption.
        last_caption = captions[-1]
        last_caption.end = helper.add_time_and_timedelta(
            last_caption.end, self._user_config["remain_time"])
        ...
        for (caption_1, caption_2) in pairwise(captions):
                ...
                caption_1.end, self._user_config["remain_time"])
    ...
    def finish(self) -> None:
        if user_config_helper.CaptioningMode.OFFLINE == self._user_config["captioning_mode"]:
            for caption in self.captions_from_offline_results():
                helper.write_to_console_or_file(text=self.string_from_caption(
                    caption), user_config=helper.Read_Only_Dict(self._user_config))
            ...
            if self._previous_caption is not None:
                self._previous_caption.end = helper.add_time_and_timedelta(
                    self._previous_caption.end, self._user_config["remain_time"])
                helper.write_to_console_or_file(text=self.string_from_caption(
                    self._previous_caption), user_config=helper.Read_Only_Dict(self._user_config))
    ...
    def initialize(self):
        ...
        if not self._user_config["use_sub_rip_text_caption_format"]:
            helper.write_to_console_or_file(text="WEBVTT{}{}".format(
                linesep, linesep), user_config=self._user_config)
        return
    ...
    def audio_config_from_user_config(self) -> helper.Read_Only_Dict:
        if self._user_config["input_file"] is None:
            ...
            if not self._user_config["use_compressed_audio"]:
                ...
                ), bits_per_sample=reader.getsampwidth() * 8, channels=reader.getnchannels())
                reader.close()
            else:
            ...
            callback = helper.BinaryFileReaderCallback(
                filename=self._user_config["input_file"])
    ...
    def speech_config_from_user_config(self) -> speechsdk.SpeechConfig:
        ...
        speech_config = speechsdk.SpeechConfig(
            subscription=self._user_config["subscription_key"].decode('utf-8'),
            region=self._user_config["region"].decode('utf-8')
        )
        ...
        speech_config.set_property(
            property_id=speechsdk.PropertyId.SpeechServiceResponse_PostProcessingOption, value="TrueText")
    ...
    def recognize_continuous(self, speech_recognizer: speechsdk.SpeechRecognizer, format: speechsdk.audio.AudioStreamFormat, callback: helper.BinaryFileReaderCallback, stream: speechsdk.audio.PullAudioInputStream):
        ...
        def recognizing_handler(e: speechsdk.SpeechRecognitionEventArgs):
            if speechsdk.ResultReason.RecognizingSpeech == e.result.reason and len(e.result.text) > 0:
                ...
                try:
                    caption = self.caption_from_real_time_result(
                        e.result, False)
                    if caption is not None:
                        helper.write_to_console_or_file(
                            text=caption, user_config=self._user_config)
                except Exception as ex:
                    print('Exception in recognizing_handler: {}'.format(ex))
            elif speechsdk.ResultReason.NoMatch == e.result.reason:
                helper.write_to_console(text="NOMATCH: Speech could not be recognized.{}".format(
                    linesep), user_config=self._user_config)
        ...
        def canceled_handler(e: speechsdk.SpeechRecognitionCanceledEventArgs):
            ...
            if speechsdk.CancellationReason.EndOfStream == e.cancellation_details.reason:
                helper.write_to_console(text="End of stream reached.{}".format(
                    linesep), user_config=self._user_config)
                done = True
            elif speechsdk.CancellationReason.CancelledByUser == e.cancellation_details.reason:
                helper.write_to_console(text="User canceled request.{}".format(
                    linesep), user_config=self._user_config)
                done = True
                ...
                print("Encountered error. Cancellation details: {}{}".format(
        ...
        def stopped_handler(e: speechsdk.SessionEventArgs):
            nonlocal done
            helper.write_to_console(text="Session stopped.{}".format(
                linesep), user_config=self._user_config)
            done = True
