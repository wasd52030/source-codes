# Source: https://github.com/HFrost0/Lighting-bilibili-download
...
class Downloader:
    def __init__(self, videos_dir='videos', sess_data='', video_concurrency=3, part_concurrency=10):
        ...
        self.client = httpx.AsyncClient(
            headers=headers, cookies=cookies, http2=True)
    ...
    async def get_favour(self, fid, num=20, keyword='', quality=0, series=True):
        ...
        """
        ps = 20
        params = {'media_id': fid, 'pn': 1, 'ps': ps,
                  'keyword': keyword, 'order': 'mtime'}
        ...
        for i in range(page_nums):
            if i + 1 == page_nums:
                ...
                num = ps
            cors.append(self.get_favor_by_page(
                fid, i + 1, num, keyword, quality, series))
    ...
    async def get_favor_by_page(self, fid, pn=1, num=20, keyword='', quality=0, series=True):
        ...
        ps = 20
        num = min(ps, num)
        params = {'media_id': fid, 'order': 'mtime',
                  'ps': ps, 'pn': pn, 'keyword': keyword}
    ...
    async def get_cate_videos(self, cate_name: str, num=10, order='click', keyword='', days=7, quality=0, series=True):
        ...
        time_from, time_to = time_from.strftime(
            '%Y%m%d'), time_to.strftime('%Y%m%d')
        pagesize = 30
        page = 1
        cors = []
        while num > 0:
            ...
            cors.append(self._get_cate_videos_by_page(
                min(pagesize, num), params, quality, series))
            num -= pagesize
            page += 1
        await asyncio.gather(*cors)
    ...
    async def _get_cate_videos_by_page(self, num, params, quality=0, series=True):
        ...
        cors = [self.get_series(f"https://www.bilibili.com/video/{i['bvid']}", quality=quality) if series else
                self.get_video(
                    f"https://www.bilibili.com/video/{i['bvid']}", quality=quality)
                for i in info]
        await asyncio.gather(*cors)
    ...
    async def get_up_videos(self, mid: str, total=10, order='pubdate', keyword='', quality=0, series=True):
        ...
        """
        ps = 30
        params = {'mid': mid, 'order': order,
                  'ps': ps, 'pn': 1, 'keyword': keyword}
        ...
        for i in range(page_nums):
            if i + 1 == page_nums:
                ...
                num = ps
            cors.append(self.get_up_videos_by_page(
                mid, i + 1, num, order, keyword, quality, series))
        await asyncio.gather(*cors)
    ...
    async def get_series(self, url: str, quality: int = 0):
        ...
        initial_state = re.search(
            r'<script>window.__INITIAL_STATE__=({.*});\(', res.text).groups()[0]
        ...
        if 'videoData' in initial_state:  # bv影片
            for idx, i in enumerate(initial_state['videoData']['pages']):
                ...
                add_names.append(i['part'] if len(
                    initial_state['videoData']['pages']) > 1 else '')
    ...
    async def get_video(self, url, quality: int = 0, add_name='', ):
        ...
        try:
            play_info = re.search(
                '<script>window.__playinfo__=({.*})</script><script>', res.text).groups()[0]
            play_info = json.loads(play_info)
            # choose quality
            for q, (_, i) in enumerate(groupby(play_info['data']['dash']['video'], key=lambda x: x['id'])):
                video_info = next(i)
                video_urls = (
                    video_info['base_url'], *(video_info['backup_url'] if video_info['backup_url'] else ()))
                if q == quality:
                    break
    ...
    async def _get_media(self, media_urls: tuple, media_name, task_id):
        ...
        self.progress.update(
            task_id, total=self.progress.tasks[task_id].total + total)
        ...
        for i in range(self.part_concurrency):
            start = i * part_length
                ...
                1 if i < self.part_concurrency - 1 else total - 1
            part_name = f'{media_name}-{start}-{end}'
            part_names.append(part_name)
            cors.append(self._get_media_part(
                media_urls, (start, end), part_name, task_id))
        await asyncio.gather(*cors)
