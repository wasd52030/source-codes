...
async def download(id, num):
    ...
    await d.get_favour(fid=id, quality=100, num=num, keyword='')
    await d.aclose()
...
def ToMp3(video):
    subprocess.run([
        ...
        os.path.join(os.getcwd(), str(html.unescape(video)).replace("mp4", "mp3"))
    ], capture_output=True)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(f"./{FavourInfo['title']}"):
