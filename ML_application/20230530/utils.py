...
class modelScore(modelInfo):
    def __init__(
        self, model: object, train: float, test: float, modelName: str = ""
    ) -> None:
...
def Logger(prefix="") -> Callable:
    def decorator(func) -> Callable:
        def wrap(*args, **kwargs):
            ...
            startMsg, endMsg = "", ""
            if prefix != "":
                startMsg = f"function {prefix}_{func.__name__}() start"
                endMsg = f"function {prefix}_{func.__name__}() end"
            else:
